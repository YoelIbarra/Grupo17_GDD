...
--[SUC_MAYOR_DESTINO]
	(SELECT TOP 1 LEFT(CI2.CIUDAD_NOMBRE, 1)+LOWER(SUBSTRING(CI2.CIUDAD_NOMBRE,2, 254))
		...
		GROUP BY SU2.SUCURSAL_ID,VU2.VUELO_RUTA_AEREA_DEST, CI2.CIUDAD_NOMBRE
		ORDER BY COUNT(VU2.VUELO_CODIGO) DESC
		),
	--	[SUC_FACTURACION_TOTAL]
	SUM(HA.HABIT_PRECIO) + SUM(PA.PASAJE_PRECIO),
	--	[SUC_CANT_VIAJES]
	SUM(DISTINCT PA.PASAJE_VUELO),
	--	[SUC_CANT_PASAJES]
	SUM(PA.PASAJE_CODIGO),
	--	[SUC_CANT_ESTADIAS]
	SUM(ES.ESTADIA_CODIGO),
	--	[SUC_MAYOR_CLIENTE]
		(SELECT TOP 1 LEFT(CL2.CLIENTE_NOMBRE, 1)+LOWER(SUBSTRING(CL2.CLIENTE_APELLIDO,2, 254)) +' '+ LEFT(CL2.CLIENTE_APELLIDO, 1)+LOWER(SUBSTRING(CL2.CLIENTE_APELLIDO,2, 254))
		FROM [DATASCIENTISTS].SUCURSAL SU2
		...
		WHERE SU.SUCURSAL_ID = SU2.SUCURSAL_ID
		GROUP BY SU2.SUCURSAL_ID,FA2.FACT_CLIENTE,CL2.CLIENTE_NOMBRE,CL2.CLIENTE_APELLIDO
		ORDER BY COUNT(FA2.FACT_NUMERO) DESC
	)
