...
END
GO
--Carga HECHOS_ESTADIAS (In Progress)
/*
SELECT 
--[ID_MES_ANIO]
(CASE
...
--[PRECIO_PROM]
--[FACTURACION]
FROM DATASCIENTISTS.FACTURA FA
	INNER JOIN DATASCIENTISTS.CLIENTE CL ON FA.FACT_CLIENTE = CL.CLIENTE_FECHA_NAC
	INNER JOIN DATASCIENTISTS.ITEMS_ESTADIA IE ON FA.FACT_NUMERO = IE.ITEM_EST_FACT_NUMERO
	INNER JOIN DATASCIENTISTS.ESTADIA ES ON IE.ITEM_EST_COD_ESTADIA = ES.ESTADIA_CODIGO
	INNER JOIN DATASCIENTISTS.COMPRA CO ON ES.ESTADIA_COMPRA = CO.COMPRA_NUMERO
	INNER JOIN DATASCIENTISTS.EMPRESA EM ON CO.COMPRA_EMPRESA = EM.EMPRESA_ID
GROUP BY YEAR(FA.FACT_FECHA),MONTH(FA.FACT_FECHA), DATEDIFF(YEAR, CL.CLIENTE_FECHA_NAC, Getdate())
